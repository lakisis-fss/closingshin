...
print(f"Starting batch fix for vcp_reports on date: {iso_date}")

try:
    # Get all vcp_reports records for that date
    r = pb.collection('vcp_reports').get_full_list(query_params={
        ...
        try:
            ...
            if not df.empty and 'Close' in df.columns:
                ...
                if int(item.close) != correct_price:
                    ...
                    pb.collection('vcp_reports').update(item.id, {"close": correct_price})
                else:
                    pass
        except Exception as e:
            print(f"  Failed for {ticker}: {e}")

    print("Batch fix for vcp_reports completed.")
